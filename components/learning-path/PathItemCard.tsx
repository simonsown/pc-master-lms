...
export function PathItemCard({ item, index }: Props) {
  const getIcon = () => {
    switch (item.item_type) {
      ...
      default: return <BookOpen size={20} className="text-[#00d4aa]" style={{ color: 'var(--brand-primary)' }} />
    }
  }
  ...
  const cardContent = (
    <motion.div
      whileHover={item.is_unlocked ? { y: -3, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' } : {}}
    ...
    `}
      style={{
        background: item.completed ? 'color-mix(in srgb, var(--brand-primary) 5%, var(--bg-surface))' : 'var(--bg-surface)',
        borderColor: item.completed ? 'color-mix(in srgb, var(--brand-primary) 40%, transparent)' : 'var(--border-default)'
      }}
    >
      <div className="flex items-center gap-4">
        ...
        `}
          style={item.completed ? { background: 'color-mix(in srgb, var(--brand-primary) 15%, transparent)', color: 'var(--brand-primary)' } : item.is_unlocked ? { background: 'var(--border-default)', color: 'var(--text-primary)' } : { background: 'var(--bg-surface)', color: 'var(--text-secondary)' }}
        >
          {index + 1}
        </div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h4 className={`font-semibold ${item.is_unlocked ? 'text-white' : 'text-gray-500'}`}
              style={{ color: item.is_unlocked ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
              {item.title}
            </h4>
          </div>
          <p className="text-xs text-gray-400 line-clamp-1 mb-2" style={{ color: 'var(--text-secondary)' }}>
            {item.description || 'Không có mô tả'}
          </p>
          <div className="flex items-center gap-3 text-[10px] font-bold text-gray-500" style={{ color: 'var(--text-secondary)' }}>
            <span>⏱️ {item.estimated_minutes} phút</span>
            {item.is_optional && <span className="bg-gray-850 px-1.5 py-0.5 rounded text-gray-400 uppercase" style={{ color: 'var(--text-secondary)' }}>Tùy chọn</span>}
            {!item.is_unlocked && (
      ...
      <div>
        {item.completed ? (
          <CheckCircle size={24} className="text-[#00d4aa]" style={{ color: 'var(--brand-primary)' }} />
        ) : item.is_unlocked ? (
