...
import { Loader2, Mail, Lock, Eye, EyeOff, X, ShieldCheck } from 'lucide-react'
...
export default function LoginModal({ isOpen, onClose, redirectTo }: LoginModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res?.error) {
      ...
      router.push(redirectTo || res.redirectUrl)
    } else {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors p-1 rounded-lg hover:bg-[#1e293b]"
        >
          <X size={20} />
        </button>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="relative">
            ...
            <input
              ...
              name="email"
              required
              className="w-full bg-[#0f0f1a] border border-[#1e293b] rounded-xl pl-11 pr-4 py-3 outline-none focus:border-[#00d2a0] text-white text-sm"
          ...
          <div className="relative">
            ...
            <input
              ...
              name="password"
              required
              className="w-full bg-[#0f0f1a] border border-[#1e293b] rounded-xl pl-11 pr-12 py-3 outline-none focus:border-[#00d2a0] text-white text-sm"
          ...
          <div className="flex items-center justify-between mt-1">
            <Link
              ...
              className="text-sm font-medium text-[#00b4d8] hover:text-[#00d2a0] transition-colors"
              onClick={onClose}
            >
              Quên mật khẩu?
            </Link>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#00d2a0] hover:bg-[#00e6af] text-black font-bold py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(0,210,160,0.15)] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
        ...
        <p className="text-center text-sm text-slate-400 mt-6">
          Chưa có tài khoản?{' '}
          <Link href="/register" onClick={onClose} className="text-[#00d2a0] font-medium hover:underline">
            Đăng ký ngay
          </Link>
        </p>
      </motion.div>
