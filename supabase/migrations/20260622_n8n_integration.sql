...
-- ----------------------------------------
-- Fix: notifications table type constraint
-- The original constraint blocked all inserts.
-- Drop and recreate with proper allowed values.
-- ----------------------------------------
...
ALTER TABLE IF EXISTS public.notifications
  ...
  CHECK (type IN ('info', 'success', 'warning', 'error', 'assignment', 'quiz', 'achievement_earned', 'lesson_completed', 'child_low_score', 'quiz_graded'));

-- ----------------------------------------
-- Add Zalo fields to profiles for teacher Zalo notifications
-- ----------------------------------------
ALTER TABLE IF EXISTS public.profiles
  ADD COLUMN IF NOT EXISTS zalo_phone TEXT,
  ADD COLUMN IF NOT EXISTS zalo_notifications BOOLEAN DEFAULT false,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'n8n_events' AND policyname = 'n8n service role only'
  ) THEN
