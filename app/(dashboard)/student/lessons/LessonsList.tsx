...
export default function LessonsList({
  ...
  const [progressList, setProgressList] = useState<Progress[]>(initialProgress)
  ...
  useEffect(() => {
    const channel = supabase
      .channel('student-lessons-realtime')
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'lessons', filter: 'is_published=eq.true' },
        (payload) => {
      ...
      .on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'lessons', filter: 'is_published=eq.true' },
        (payload) => {
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          const p = payload.new as Progress
          setProgressList(prev => {
            const idx = prev.findIndex(x => x.lesson_id === p.lesson_id)
            if (idx >= 0) {
              const next = [...prev]
              next[idx] = p
              return next
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)' }}>
          ...
          <div>
            ...
            <div className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{completedCount}/{lessons.length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {lessons.length === 0 && (
          <div className="col-span-full py-20 text-center rounded-2xl border-2 border-dashed" style={{ borderColor: 'var(--border-subtle)' }}>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Bài giảng mới sẽ xuất hiện ở đây ngay khi giáo viên đăng.</p>
          </div>
        )}
      </div>
    </div>
